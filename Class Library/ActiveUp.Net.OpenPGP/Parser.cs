...
namespace ActiveUp.Net.Security.OpenPGP
{
    public static class Parser
    {
        ...
        private static Packet DispatchPacket(Packet packet)
        {
            ...
            object pack = null;
#if !PocketPC
            pack = Activator.CreateInstance("ActiveUp.Mail", "ActiveUp.Net.Security.OpenPGP.Packets." + packet.Type.ToString()).Unwrap();
#else
            pack = Activator.CreateInstance(t);
#endif
        ...
        private static Packet GetNextPacket(ref MemoryStream stream)
        {
            ...
            if (!first[0].Equals('1'))
                throw new InvalidPacketSyntaxException("First byte's first bit is 0.");

            PacketFormat format = (PacketFormat)int.Parse(first[1].ToString());
            ...
            if (format.Equals(PacketFormat.Old))
            {
...
#if !PocketPC
                packet = (Packet)(Activator.CreateInstance("ActiveUp.Mail", "ActiveUp.Net.Security.OpenPGP.Packets."+type.ToString()).Unwrap());
#else
                packet = (Packet)(Activator.CreateInstance(Type.GetType("ActiveUp.Net.Security.OpenPGP.Packets."+type.ToString())));
#endif
                ...
                else if (lengthType == 2)
                {
                    ...
                    packet.TotalLength = packet.BodyLength + 1;
                else
                    throw new InvalidPacketSyntaxException("Invalid old format packet length type : "+lengthType.ToString());
