...
namespace ActiveUp.Net.Mail.PDI.vCard
{
	...
	{
		public static ActiveUp.Net.Mail.PDI.vCard.vCard Parse(string data)
		{
			ActiveUp.Net.Mail.PDI.vCard.vCard card = new ActiveUp.Net.Mail.PDI.vCard.vCard();
		...
		public static ActiveUp.Net.Mail.PDI.vCard.vCard Parse(byte[] data)
		{
			return ActiveUp.Net.Mail.PDI.vCard.Parser.Parse(System.Text.Encoding.UTF7.GetString(data));
		}
		...
		{
			string val = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
			string type = line.Split(':')[0].ToUpper();
			if(type.IndexOf("ENCODING=QUOTED-PRINTABLE")!=-1) val = FromQuotedPrintable(val,"utf-8");
			if(type.IndexOf("ENCODING=B")!=-1) val = System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String(val));
			string[] values = val.Split(';');
			ActiveUp.Net.Mail.PDI.vCard.Address adr = new ActiveUp.Net.Mail.PDI.vCard.Address();
			if(values[0].Length>0) adr.POBox = System.Convert.ToInt32(values[0]);
			if(values[1].Length>0) adr.ExtendedAddress = values[1];
			if(values[2].Length>0) adr.StreetAddress = values[2];
			if(values[3].Length>0) adr.Locality = values[3];
			if(values[4].Length>0) adr.Region = values[4];
			if(values[5].Length>0) adr.PostalCode = values[5];
			if(values[6].Length>0) adr.Country = values[6];
		...
		{
			string val = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
			ActiveUp.Net.Mail.PDI.vCard.TelephoneNumber tel = new ActiveUp.Net.Mail.PDI.vCard.TelephoneNumber();
			tel.Number = val;
			string parameters = line.Split(':')[0].ToUpper();
		...
		private static void AddLabel(vCard card, string line)
		{
			...
			if(type.IndexOf("ENCODING=QUOTED-PRINTABLE")!=-1) val = FromQuotedPrintable(val,"utf-8");
			if(type.IndexOf("ENCODING=B")!=-1) val = System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String(val));
			ActiveUp.Net.Mail.PDI.vCard.Label label = new ActiveUp.Net.Mail.PDI.vCard.Label();
			label.Value = val;
			string parameters = line.Split(':')[0].ToUpper();
		...
		private static void AddEmail(vCard card, string line)
		{
			...
			ActiveUp.Net.Mail.PDI.vCard.EmailAddress email = new ActiveUp.Net.Mail.PDI.vCard.EmailAddress();
			email.Address = val;
			string parameters = line.Split(':')[0].ToUpper();
		...
		{
			card.FullName = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetDisplayName(vCard card, string line)
		{
			card.DisplayName = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetName(vCard card, string line)
		{
			...
			if(values.Length>0) card.Name.FamilyName = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(values[0]);
			if(values.Length>1) card.Name.GivenName = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(values[1]);
			if(values.Length>2) card.Name.AdditionalNames = ActiveUp.Net.Mail.PDI.vCard.Parser.UnescapeArray(values[2].Split(','));
			if(values.Length>3) card.Name.Prefixes = ActiveUp.Net.Mail.PDI.vCard.Parser.UnescapeArray(values[3].Split(','));
			if(values.Length>4) card.Name.Suffixes = ActiveUp.Net.Mail.PDI.vCard.Parser.UnescapeArray(values[4].Split(','));
		}
		private static void SetNickName(vCard card, string line)
		{
			card.Nickname = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetSource(vCard card, string line)
		{
			card.Source = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetMailer(vCard card, string line)
		{
			card.Mailer = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetTimeZone(vCard card, string line)
		{
			card.TimeZone = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetTitle(vCard card, string line)
		{
			card.Title = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetRole(vCard card, string line)
		{
			card.Role = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetNote(vCard card, string line)
		{
			card.Note = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetGeneratorId(vCard card, string line)
		{
			card.GeneratorId = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetSortText(vCard card, string line)
		{
			card.SortText = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetUid(vCard card, string line)
		{
			card.Uid = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetUrl(vCard card, string line)
		{
			card.Url = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetVersion(vCard card, string line)
		{
			card.Version = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetAccessClass(vCard card, string line)
		{
			card.AccessClass = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":",""));
		}
		private static void SetBirthday(vCard card, string line)
		{
			card.Birthday = Parser.ParseDate(ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":","")));
		}
		private static void SetRevision(vCard card, string line)
		{
			card.Revision = Parser.ParseDate(ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(line.Replace(line.Split(':')[0]+":","")));
		}
		...
		{
			for(int i=0;i<input.Length;i++) input[i] = ActiveUp.Net.Mail.PDI.vCard.Parser.Unescape(input[i]);
			return input;
		}
